...
const followsCollection = require('../db').db().collection("follows"); //create follows collection in db
...
Follow.getFollowersById = function(id) {
    return new Promise(async (resolve, reject) => {
        try {
            let followers = await followsCollection.aggregate([ //aggregate is native to mongodb
                {$match: {followedId: id}},
            ...
            followers = followers.map((follower) => {
                //create a user
                let user = new User(follower, true) //figures out a grativar based on user's email address
